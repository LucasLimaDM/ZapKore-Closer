...
export async function persistInboundMessage(
  ...
  const {
    effectiveJid, effectivePhone, pushName, messageId, fromMe,
    text, type, timestamp, raw, evoUrl = '', evoKey = '',
    instanceName = '', remoteJidLidHint,
  } = msg
  ...
  if (!contact) {
    ...
    if (!fromMe && pushName && pushName !== 'Unknown' && !/^\d+$/.test(pushName) &&
  ...
  if (type === 'audioMessage' || type === 'pttMessage') {
    ...
    if (newVersion != null) {
      const audioTask = processAudioMessageFn(
        ...
        newVersion as number, evoUrl, evoKey, instanceName, remoteJidLidHint,
      )
      if (typeof (globalThis as any).EdgeRuntime?.waitUntil === 'function') {
    ...
    if (newVersion != null) {
      const aiTask = processAiResponseFn(
        userId, contact.id, supabaseUrl, supabaseKey, newVersion as number, remoteJidLidHint,
      )
      if (typeof (globalThis as any).EdgeRuntime?.waitUntil === 'function') {
          ...
          if (!fromMe && remoteJid?.includes('@lid') && remoteJidAlt?.includes('@s.whatsapp.net')) {
            ...
            if (/^\d{8,15}$/.test(altPhone)) {
              const linkPromise = linkLidToPhone(supabase, {
                userId, instanceId: integ.id, lidJid: remoteJid, phoneJid: remoteJidAlt,
            ...
            if (type === 'audioMessage' || type === 'pttMessage') {
              ...
              if (newVersion != null) {
                ...
                const audioTask = processAudioMessage(
                  userId, contact.id, messageId, supabaseUrl, supabaseKey,
                  newVersion as number, evoUrl, evoKey || '', instanceName, lidHint,
                )
                if (typeof (globalThis as any).EdgeRuntime?.waitUntil === 'function') {
                  ;(globalThis as any).EdgeRuntime.waitUntil(audioTask)
                } else {
              ...
              if (newVersion != null) {
                ...
                if (typeof (globalThis as any).EdgeRuntime?.waitUntil === 'function') {
                  ;(globalThis as any).EdgeRuntime.waitUntil(
                    processAiResponse(userId, contact.id, supabaseUrl, supabaseKey, newVersion as number, lidHint),
                  )
                } else {
                  processAiResponse(userId, contact.id, supabaseUrl, supabaseKey, newVersion as number).catch(
                    (e: any) => console.error('[WEBHOOK] AI task failed:', e),
                  )
                }
              }
            }
...
```typescript
    const provider = getProvider(integration)
    await provider.sendText(contact.remote_jid, integration.rate_limit_message ?? 'Identificamos um volume elevado de mensagens e transferiremos seu atendimento para um de nossos atendentes. Em breve você será atendido!').catch(
      (err: any) => console.error(`[AI Handler] rate_limit_msg_send_failed contactId=${contactId}:`, err),
    )
```
...
```typescript
    ...
    await providerFallback.sendText(contact.remote_jid, fallbackText).catch(
      (err: any) => console.error(`[AI Handler] fallback_send_failed contactId=${contactId}:`, err),
    )
```
...
```typescript
    ...
    if ('connected' in result) {
      ...
      const webhookUrl = integ.provider === 'zapi'
...
```typescript
    ...
    try {
      ...
      if (connected) return new Response(JSON.stringify({ success: true, connected: true }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
...
```typescript
    const provider = getProvider(integration)
    const rawChats = await provider.syncChats()
    const chats = rawChats.map((c) => ({ remoteJid: c.remoteJid, jid: c.remoteJid, pushName: c.pushName, name: c.pushName }))
```
...
```typescript
    if (action === 'get') {
      const { data: integ } = await supabaseAdmin
        .from('user_integrations')
        .select('provider, evolution_api_url, evolution_api_key, zapi_instance_id, zapi_instance_token, zapi_client_token')
        .eq('user_id', user.id)
        .single()
...
```typescript
    if (action === 'save') {
      const { provider = 'evolution', url, api_key, zapi_instance_id, zapi_instance_token, zapi_client_token } = body
...
```tsx
const body = editProvider === 'zapi'
  ? { action: 'save', provider: 'zapi', zapi_instance_id: zapiInstanceId, zapi_instance_token: zapiInstanceToken, zapi_client_token: zapiClientToken }
